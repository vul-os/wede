<!-- Generated from docs/DEPLOYMENT.md by scripts/sync-docs.mjs — edit the source, not this file. -->
...
It assumes you've already read [GETTING-STARTED.md](#getting-started) and
have a working `wede.config.json`. For TLS / reverse proxies / tunnels, see
[PUBLIC-ACCESS.md](#public-access); for the full config key reference, see
[CONFIGURATION.md](#configuration).

> wede's [README](#overview) currently carries a **"deprioritized" /
...
— [ARCHITECTURE.md § No-sandbox warning](#architecture:no-sandbox-warning-for-shared-deployments),
also called out in the [README](https://github.com/vul-os/wede/blob/main/README.md#collaboration).
...
- `--shell /usr/sbin/nologin` stops *you* from `su -`-ing into it casually, but
  ...
  unset — see [terminal.go](https://github.com/vul-os/wede/blob/main/backend/internal/terminal/terminal.go)), not
...
[CONFIGURATION.md § workspace_root](#configuration:workspace_root) for the
...
[README](https://github.com/vul-os/wede/blob/main/README.md#quick-start)) and place it somewhere stable, e.g.
...
coupling (see [CONFIGURATION.md § Config file location](#configuration:config-file-location)
...
[security model](#architecture:no-sandbox-warning-for-shared-deployments).
...
| `lockout.json` | Failed-login attempt counter + locked flag (`auth.go`). | No. | No. | Resets the brute-force lockout — see [TROUBLESHOOTING.md](#troubleshooting:locked-out-after-failed-logins) for the exact recovery steps (a file delete alone is not enough; the process must restart). |
...
the [CHANGELOG](#changelog) for that release rather than assuming a bug.
...
- [PUBLIC-ACCESS.md](#public-access) — TLS, reverse proxies, and tunnel
  options for reaching wede from outside `localhost`.
- [CONFIGURATION.md](#configuration) — every config key, in full.
- [TROUBLESHOOTING.md](#troubleshooting) — symptom → cause → fix for the
