...
func findDist() string {
	cwd, _ := os.Getwd()
	// Check cwd and walk up to find web/dist (handles running from backend/).
	// The frontend project (and its `vite build` output) lives under web/,
...
func newFrontendHandler() http.HandlerFunc {
	...
	if distDir == "" {
		log.Printf("[dev] no web/dist found - use Vite dev server (localhost:5173) or run 'npm run build' in web/")
		return func(w http.ResponseWriter, r *http.Request) {
			...
			w.Write([]byte(`<!doctype html><html><body style="background:#111827;color:#f1f5f9;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0">
				<div style="text-align:center">
					...
					<p style="color:#94a3b8;font-size:14px">Run <code style="color:#60a5fa">npm run dev</code> in web/ in another terminal</p>
				</div>
			</body></html>`))
		}
	}
