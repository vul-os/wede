...
match what you're seeing, check the [CHANGELOG](../CHANGELOG.md) for your
...
2. Delete (or reset) the lockout file:
   ...
   [DEPLOYMENT.md § State & backup](DEPLOYMENT.md#5-state--backup) if you're
...
[CONFIGURATION.md § CLI flags](CONFIGURATION.md#cli-flags).)
...
[PUBLIC-ACCESS.md](PUBLIC-ACCESS.md) — binding to `0.0.0.0` puts a
...
`.wede/lsp.json`; [GETTING-STARTED.md](GETTING-STARTED.md#workspace-trust)
...
[PUBLIC-ACCESS.md § Direct bind + reverse proxy](PUBLIC-ACCESS.md#a-direct-bind--reverse-proxy)
...
[CONFIGURATION.md § frame_ancestors](CONFIGURATION.md#frame_ancestors).
...
- **Editor link "works" but the person doesn't get a shell/writes** — check
  ...
  [ARCHITECTURE.md § Role model](ARCHITECTURE.md#role-model).
...
[CONFIGURATION.md § workspace_root](CONFIGURATION.md#workspace_root) and
[DEPLOYMENT.md § workspace_root](DEPLOYMENT.md#workspace_root-what-it-confines-and-what-it-doesnt)
...
- [DEPLOYMENT.md](DEPLOYMENT.md) — running wede as a long-lived service,
  what state it keeps, and how to back it up.
- [PUBLIC-ACCESS.md](PUBLIC-ACCESS.md) — exposing wede beyond `localhost`
  safely.
- [CONFIGURATION.md](CONFIGURATION.md) — every config key, defaults, and
  precedence rules.
- [ARCHITECTURE.md](ARCHITECTURE.md) — the security/role model in full.
