...
type GitClassification =
  | { type: 'commit'; hash: string; subject: string }
  | { type: 'churn'; text: string }

// A message annotated with how it should render (set by the `rows` memo below).
type ChatRow = ChatMessage & {
  _row: 'user' | 'commit' | 'churn'
  _grouped?: boolean
  hash?: string
  subject?: string
}
...
function classifyGit(text: string | undefined): GitClassification {
...
function CommitRow({ hash, subject }: { hash?: string; subject?: string }) {
  return (
...
function ChurnRow({ text }: { text?: string }) {
  return (
...
function UserMessage({ msg, grouped }: { msg: ChatRow; grouped?: boolean }) {
...
export default function Chat({ workspaceId, workspaceName, token, username, color }: ChatProps) {
