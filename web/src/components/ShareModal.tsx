 ...
 */
export default function ShareModal({ authFetch, onClose }: ShareModalProps) {
  ...
  const fetchTokens = useCallback(async () => {
    ...
    try {
      ...
      const data: { tokens?: InviteToken[] } = await res.json()
  ...
  const handleMint = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data: { error?: string; raw?: string } = await res.json()
      if (!res.ok) {
