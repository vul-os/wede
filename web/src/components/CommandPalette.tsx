import { useState, useEffect, useRef, useCallback, type ComponentType, type KeyboardEvent, type ReactNode } from 'react'
import {
...
// lucide-react doesn't export its `LucideIcon` component type, so this
// structurally matches how every icon is used here (just `className`).
...
export default function CommandPalette({
  ...
  /* ── Command definitions ── */
  const rawCommands: (Command | false)[] = [
    hasActiveTab && {
      id: 'save',
      label: 'Save File',
  ...
  const commands: Command[] = rawCommands.filter((c): c is Command => Boolean(c))

  /* ── Filtered + ranked results ── */
  const results: ScoredCommand[] = commands
