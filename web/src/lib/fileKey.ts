...
export function fileKey(workspaceId: string | null | undefined, rel: string | null | undefined): string {
...
export function parseFileKey(key: unknown): ParsedFileKey {
  const i = typeof key === 'string' ? key.indexOf(KEY_SEP) : -1
  if (i === -1) return { workspaceId: null, rel: typeof key === 'string' ? key : '' }
  return { workspaceId: (key as string).slice(0, i), rel: (key as string).slice(i + 1) }
}

export function isFileKey(key: unknown): boolean {
...
export function isSpecialTabId(id: unknown): boolean {
...
type NormalizedTab = Partial<Tab> & { path: string }

export function normalizeTabIdentity(
  tab: Partial<Tab> | null | undefined,
  activeWorkspaceId?: string | null,
): NormalizedTab | null | undefined {
  if (!tab || tab.type || isSpecialTabId(tab.path)) return tab as NormalizedTab | null | undefined
  ...
  if (!workspaceId) return tab as NormalizedTab // can't resolve a root yet — leave as-is
  const path = fileKey(workspaceId, rel)
  if (tab.path === path && tab.rel === rel && tab.workspaceId === workspaceId) return tab as NormalizedTab
