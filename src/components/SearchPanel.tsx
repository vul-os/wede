import { useState, useCallback, useRef, useMemo, type KeyboardEvent, type ChangeEvent, type RefObject } from 'react'
import {
...
import type { AuthFetch, WorkspaceSummary, FileEntry } from '../types'
...
interface ReplacePreviewResponse {
  error?: string
  matches?: SearchMatch[]
}
...
type SearchRoot = Pick<WorkspaceSummary, 'id' | 'name'>
...
const groupKey = (m: SearchMatch) => `${m.workspaceId || ""}${NUL}${m.file}`
...
export interface SearchPanelProps {
  authFetch: AuthFetch
  onOpenFile?: (entry: FileEntry, line?: number) => void
  readOnly?: boolean
  workspaces?: WorkspaceSummary[]
  workspaceId?: string | null
}

export default function SearchPanel({ authFetch, onOpenFile, readOnly = false, workspaces = [], workspaceId }: SearchPanelProps) {
  ...
  const roots: SearchRoot[] = useMemo(() => (
  ...
  // ── result state ────────────────────────────────────────────────────────────
  const [results, setResults]       = useState<SearchMatch[] | null>(null) // null = no search yet; [] = empty
  const [fileResults, setFileResults] = useState<FileMatch[] | null>(null)
  ...
  const [previewResults, setPreviewResults]   = useState<SearchMatch[] | null>(null)
  ...
  // ── core search ─────────────────────────────────────────────────────────────
  const doSearch = useCallback(async (q: string, cs: boolean, ww: boolean, rx: boolean, inc: string, exc: string, mode: SearchMode) => {
    if (!q.trim()) {
      setResults(null)
      setFileResults(null)
      setError('')
      ...
      if (mode === 'files') {
        ...
        const per = await Promise.all(roots.map(async (ws) => {
          try {
            ...
            const data: SearchFilesResponse = await res.json()
            if (data.error) return { error: data.error }
            return { files: (data.files || []).map((f) => ({ ...f, workspaceId: ws.id, rootName: ws.name })), count: data.count || 0, truncated: !!data.truncated }
          } catch (e) { if ((e as Error).name === 'AbortError') throw e; return { error: 'search failed' } }
        }))
  ...
  const triggerSearch = useCallback((q: string, cs: boolean, ww: boolean, rx: boolean, inc: string, exc: string, mode: SearchMode) => {
  ...
  const handleIncludeChange = (e: ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value
    setIncludeGlob(v)
  ...
  const handleFileResultClick = (fileMatch: FileMatch) => {
  ...
  const handleResultKeyDown = (e: KeyboardEvent<HTMLButtonElement>, idx: number) => {
    if (e.key === 'ArrowDown') {
      e.preventDefault()
    ...
    try {
      ...
      const per = await Promise.all(roots.map(async (ws) => {
        try {
          ...
          const data: ReplacePreviewResponse = await res.json()
          if (data.error) return { error: data.error }
      ...
      setReplaceError((e as Error).message || 'Preview failed')
    }
    ...
    try {
      ...
      const per = await Promise.all(roots.map(async (ws) => {
        try {
          ...
          const data: ReplaceAllResponse = await res.json()
          if (data.error) return { error: data.error }
  ...
  const summaryText = (() => {
    if (isPreviewMode) {
      const n = displayResults?.length ?? 0
      return `Preview: ${n} replacement${n !== 1 ? 's' : ''}`
    }
    if (searchMode === 'files') {
...
interface FileGroupProps {
  ...
  onResultKeyDown: (e: KeyboardEvent<HTMLButtonElement>, idx: number) => void
  resultButtonsRef: RefObject<(HTMLButtonElement | null)[]>
  isPreview: boolean
  totalMatches?: number
}

function FileGroup({ file, rootName, matches, groupStartIdx, onResultClick, onResultKeyDown, resultButtonsRef, isPreview }: FileGroupProps) {
          ...
          {matches.map((m, i) => {
            ...
            return (
              <MatchLine
                ...
                onKeyDown={(e: KeyboardEvent<HTMLButtonElement>) => onResultKeyDown(e, globalIdx)}
                buttonRef={(el: HTMLButtonElement | null) => { resultButtonsRef.current[globalIdx] = el }}
                isPreview={isPreview}
              />
            )
          })}
...
function MatchLine({ match, onClick, onKeyDown, buttonRef, isPreview }: MatchLineProps) {
      ...
      {match.before && match.before.map((ctx, i) => (
        <div key={`b${i}`} className="flex items-baseline gap-2 px-4 py-0.5 overflow-hidden">
          <span className="shrink-0 font-mono text-[10px] text-text-muted/40 w-7 text-right select-none">
            {match.line - (match.before?.length ?? 0) + i}
          </span>
...
function FileResultRow({ fileMatch, rootName, idx, query, caseSensitive, onClick, onKeyDown, buttonRef }: FileResultRowProps) {
...
function groupByFile(matches: SearchMatch[]): FileGroupData[] {
 ...
 */
function highlightText(text: string, query: string, caseSensitive: boolean) {
