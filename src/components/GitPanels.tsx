...
import type { AuthFetch, WorkspaceSummary } from '../types'
...
export default function GitPanels({ authFetch, workspaces = [], visible, readOnly = false, onOpenGraph, workspaceId }: GitPanelsProps) {
  const roots: WorkspaceSummary[] = workspaces.length ? workspaces : (workspaceId ? [{ id: workspaceId, name: '' }] : [])
...
function RepoSection({ ws, authFetch, parentVisible, readOnly, onOpenGraph }: RepoSectionProps) {
