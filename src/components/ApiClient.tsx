...
import { buildSend, type ApiKeyValue, type ApiAuth, type ApiBody } from '../lib/apiRequest'
...
// The request/response shapes here are always fully-populated in practice —
// req comes from blankRequest() (see hooks/useApiClient.ts), which sets every
// field — so this component works with a narrower, all-required view of the
// looser ApiRequest type the hook stores.
interface FullApiRequest {
  name: string
  method: string
  url: string
  params: ApiKeyValue[]
  headers: ApiKeyValue[]
  auth: ApiAuth & { type: NonNullable<ApiAuth['type']> }
  body: ApiBody & { type: NonNullable<ApiBody['type']>; form: ApiKeyValue[] }
}
...
function KVEditor({ rows, onChange, readOnly }: KVEditorProps) {
  const set = (i: number, patch: Partial<ApiKeyValue>) => onChange(rows.map((r, j) => (j === i ? { ...r, ...patch } : r)))
...
export default function ApiClient({ api, authFetch, readOnly = false }: ApiClientProps) {
  const { setReq: setReqRaw, vars, base, saveRequest } = api
  // api.req is always fully-populated (see FullApiRequest above).
  const req = api.req as FullApiRequest
  const setReq = setReqRaw as unknown as (updater: (r: FullApiRequest) => FullApiRequest) => void
  ...
  const patch = (p: Partial<FullApiRequest>) => setReq((r) => ({ ...r, ...p }))
  const setBody = (p: Partial<FullApiRequest['body']>) => setReq((r) => ({ ...r, body: { ...r.body, ...p } }))
  const setAuth = (p: Partial<FullApiRequest['auth']>) => setReq((r) => ({ ...r, auth: { ...r.auth, ...p } }))
  ...
  const statusPill = ok ? 'bg-green/12 text-green' : (resp?.status ?? 0) >= 400 ? 'bg-red/12 text-red' : 'bg-yellow/12 text-yellow'
      ...
      <div className="px-3 py-3 border-b border-border max-h-52 overflow-y-auto shrink-0">
        ...
        {reqTab === 'auth' && (
          <div className="space-y-2 max-w-lg">
            <select value={req.auth.type} disabled={readOnly} onChange={(e) => setAuth({ type: e.target.value as FullApiRequest['auth']['type'] })}
