import { useState, useEffect, useCallback, useMemo, useRef, type MouseEvent as ReactMouseEvent, type ReactNode, type FormEvent, type ComponentType } from 'react'
import {
...
import { buildGraph, type GitCommitEntry, type GitMergeLine } from '../lib/gitGraph'
...
// The backend's git log entries carry more than the bare {hash,parents} that
// lib/gitGraph.ts's buildGraph() needs — extend its generic entry/row shapes
// locally with the fields this panel actually renders.
interface GitLogEntry extends GitCommitEntry {
  short: string
  message: string
  author: string
  date: string
...
interface GitLogRow extends GitLogEntry {
  lane: number
  mergeLines: GitMergeLine[]
  laneCount: number
  activeLanes: (string | null)[]
}

type DiffLineType = 'add' | 'del' | 'hunk' | 'meta' | 'ctx' | 'empty'
...
type CommitActionName = 'checkout' | 'branchHere' | 'cherryPick' | 'revert' | 'resetSoft' | 'resetHard'
type OnCommitAction = (action: CommitActionName, hash: string, extra?: string) => void
...
type GitFileStatus = 'modified' | 'added' | 'deleted' | 'untracked' | 'copied' | 'renamed' | 'conflict'
...
// errMessage mirrors the untyped original's `err.message || fallback` — a
// non-Error thrown value (or an Error with an empty message) falls back.
...
function parseDiff(text: string | null | undefined): DiffLine[] {
  if (!text) return []
...
function buildSideBySidePairs(diffLines: DiffLine[]): DiffPair[] {
...
function SideBySideDiff({ lines }: { lines: DiffLine[] }) {
  ...
  function cellCls(type: DiffLineType) {
...
function BlamePanel({ filePath, authFetch }: { filePath: string; authFetch: AuthFetch }) {
...
interface CommitMenuProps {
  ...
  commit: GitLogRow
  onClose: () => void
  onAction: (action: CommitActionName, hash: string) => void
  readOnly: boolean
}

function CommitMenu({ x, y, commit, onClose, onAction, readOnly }: CommitMenuProps) {
...
interface BranchHereModalProps {
  hash: string
  onClose: () => void
  onConfirm: (name: string) => void
}

function BranchHereModal({ hash, onClose, onConfirm }: BranchHereModalProps) {
...
interface GraphRowProps {
  row: GitLogRow
  nextRow: GitLogRow | undefined
  isLast: boolean
  onContextMenu: (e: ReactMouseEvent, row: GitLogRow) => void
  isSelected: boolean
  onClick: (row: GitLogRow) => void
}

function GraphRow({ row, nextRow, isLast, onContextMenu, isSelected, onClick }: GraphRowProps) {
...
function splitDiffByFile(diff: string | null | undefined): DiffFileSummary[] {
  if (!diff) return []
...
interface CommitDetailProps {
  commit: GitLogRow | null
  authFetch: AuthFetch
  onClose: () => void
}

function CommitDetail({ commit, authFetch, onClose }: CommitDetailProps) {
  const [data, setData] = useState<CommitDiffResponse | null>(null)
  const [loading, setLoading] = useState(true)
...
export function GitGraph({ entries, authFetch, onCommitAction, readOnly, totalCount, onLoadMore, loadingMore }: GitGraphProps) {
  const rows = useMemo(() => buildGraph(entries) as GitLogRow[], [entries])
  const [menu, setMenu] = useState<CommitMenuState | null>(null)
  const [selected, setSelected] = useState<GitLogRow | null>(null)
  const [branchHereHash, setBranchHereHash] = useState<string | null>(null)

  const handleCtx = (e: ReactMouseEvent, row: GitLogRow) => {
    e.preventDefault()
    setMenu({ x: e.clientX, y: e.clientY, commit: row })
  }

  const handleClick = (row: GitLogRow) => {
...
function FileBadge({ status }: { status: GitFileStatus }) {
...
function parseHunks(diffText: string | null | undefined, filePath: string): DiffHunk[] {
  if (!diffText) return []
...
function FileDiffPanel({ file, staged, authFetch, onRefresh, readOnly }: FileDiffPanelProps) {
  ...
  useEffect(() => {
    ...
    authFetch(url)
      .then((r) => r.json())
      .then((data: GitDiffResponse) => { setDiffText(data.diff || ''); setLoading(false) })
  ...
  const hunkHeaderToIdx: Record<string, number> = {}
...
type ConflictChoice = 'current' | 'incoming' | 'both'

interface ConflictResponse {
  regions?: ConflictRegion[]
}

function ConflictResolver({ file, authFetch, onRefresh }: ConflictResolverProps) {
  const [regions, setRegions] = useState<ConflictRegion[] | null>(null)
  const [loading, setLoading] = useState(true)
  const [resolutions, setResolutions] = useState<Record<number, ConflictChoice>>({})
  ...
  useEffect(() => {
    ...
    authFetch(`/api/git/conflict?file=${encodeURIComponent(file.path)}`)
      .then((r) => r.json())
      .then((data: ConflictResponse) => { setRegions(data.regions || []); setLoading(false) })
...
function FileRow({ file, action, onAction, onDiscard, authFetch, onRefresh, readOnly }: FileRowProps) {
  ...
  return (
    <div>
      ...
      >
        ...
        <span className={`group-hover:hidden w-4 text-center text-[12px] font-bold shrink-0 ${(STATUS_META[file.status as GitFileStatus] || STATUS_META.modified).color}`}>
          {(STATUS_META[file.status as GitFileStatus] || STATUS_META.modified).label}
        </span>
      </div>
...
function SectionHeader({ label, count, colorClass, defaultOpen = true, children, onStageAll, onUnstageAll, readOnly }: SectionHeaderProps) {
...
function StashSection({ stashes, authFetch, onRefresh, onToast, readOnly }: StashSectionProps) {
  ...
  const handleStash = async () => {
    ...
    try {
      ...
      onToast(errMessage(e, 'Stash failed'), 'error')
    }
    setStashing(false)
  }
  ...
  const handlePop = async (index: number) => {
    ...
    try {
      ...
      onToast(errMessage(e, 'Pop failed'), 'error')
    }
    setPopping(null)
  }
...
function TagsSection({ authFetch, onToast, readOnly }: TagsSectionProps) {
...
interface GitStashesResponse {
  stashes?: GitStash[]
}

export default function GitPanel({ authFetch, visible, readOnly = false, onOpenGraph }: GitPanelProps) {
  const [status, setStatus]     = useState<GitStatus>({ branch: '', files: [], isRepo: true })
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      const [sData, bData, rData, stData]: [GitStatus, GitBranchesResponse, GitRemotesResponse, GitStashesResponse] = await Promise.all([
  ...
  const runRemoteOp = async (op: string) => {
    if (readOnly) return
    setRemoteOp(op)
    setRemoteMsg('')
    try {
      ...
      const data: ErrorResponse & { output?: string } = await res.json()
      if (data.error) {
  ...
  const handleAddRemote = async (e: FormEvent) => {
    e.preventDefault()
...
function RemoteOpBtn({ icon: Icon, label, desc, loading, disabled, onClick }: RemoteOpBtnProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
...
interface PanelHeaderProps {
  ...
  refreshing: boolean
  readOnly?: boolean
  // Unused by this component — a vestigial prop passed by the "not a repo"
  // render path below; kept (typed, but ignored) to avoid changing that call site.
  branch?: string | null
  section?: string
...
function PanelHeader({ onRefresh, refreshing, readOnly, section, setSection, onOpenGraph }: PanelHeaderProps) {
