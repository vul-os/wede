...
import { GitGraph, type GitGraphProps } from './GitPanel'
import type { AuthFetch } from '../types'

const PAGE = 100

type LogEntry = GitGraphProps['entries'][number]
...
type CommitAction = 'checkout' | 'branchHere' | 'cherryPick' | 'revert' | 'resetSoft' | 'resetHard'

export default function GitGraphView({ authFetch, readOnly = false }: { authFetch: AuthFetch; readOnly?: boolean }) {
  const [log, setLog] = useState<LogEntry[]>([])
  const [count, setCount] = useState(PAGE)
  const [loadingMore, setLoadingMore] = useState(false)
  const [refreshing, setRefreshing] = useState(false)
  const [op, setOp] = useState('')          // running fetch/pull/push label
  const [opMsg, setOpMsg] = useState<OpMessage | null>(null)
  ...
  const onCommitAction = async (action: CommitAction, hash: string, extra?: string) => {
    if (readOnly) return
    const post = (url: string, body: Record<string, unknown>) => authFetch(url, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
    })
    const actions: Record<CommitAction, () => Promise<Response>> = {
