...
import { parseReq, type ApiTreeNode } from '../lib/apiRequest'
...
interface TreeNodeProps {
  node: ApiTreeNode
  depth: number
  onOpen: (node: ApiTreeNode) => void
  onDelete: (node: ApiTreeNode) => void
  readOnly: boolean
  activePath: string | null
}

function TreeNode({ node, depth, onOpen, onDelete, readOnly, activePath }: TreeNodeProps) {
...
export interface ApiCollectionsProps {
  api: ReturnType<typeof useApiClient>
  readOnly?: boolean
  onOpenRequest?: () => void
}

export default function ApiCollections({ api, readOnly = false, onOpenRequest }: ApiCollectionsProps) {
  const open = (node: ApiTreeNode) => { api.openRequest(node); onOpenRequest?.() }
